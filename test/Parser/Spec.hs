...
spec = do
    ...
    describe "top level function and data definitions" $ do
        it "PASSES on good parser input" $ do
            ...
            parseTld "eq=typeclass:eq=func(a:string):string{a}" `shouldBe` (Right (TypeclassDef (Identifier "eq") [FuncDefUnary (Identifier "eq") (Identifier "a") (Type (Identifier "string")) (ExpVariable (Identifier "a")) (Type (Identifier "string"))]))
